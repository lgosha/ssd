...
XMLConfig::XMLConfig(const QByteArray &strBaseFileName, bool validate) : m_aptrXMLDOMParser(new XercesDOMParser),/* m_pNode(0),*/ m_bValidate(validate) {
...
void XMLConfig::open() {
	...
	if( m_bValidate ) m_aptrXMLDOMParser->setValidationScheme(XercesDOMParser::Val_Always);
...
const DOMNode* XMLConfig::first(const DOMNode *pParentNode) const {

	if( pParentNode ) return pParentNode->getFirstChild();
		
	return root()->getFirstChild();
}

const DOMNode* XMLConfig::next(const DOMNode *pParentNode) const {

	if( pParentNode ) return pParentNode->getNextSibling();

	return 0;
} 

const DOMNode* XMLConfig::getNodeAttribute(const DOMNode* pNode, const QString &sName) {
	...
	while(pNode) {
		...
		if(pNode->getNodeType() == DOMNode::ELEMENT_NODE) {
			...
			if(sAttrNameForSearch.isEmpty()) { 
				...
				const DOMNode *pAttrNode = getNodeAttribute(pNode, sAttrNameForSearch);
