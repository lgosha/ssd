...
int CCommand::exec( const QString &proc, const QString &cmd, const QMap<QString, QString> &vars, bool bSimulate ) {
	...
	if( m_pProcess->exitCode() ) return 5;
	...
	m_pCmdExecutor	( new CCommand( pXMLConfig, this ) )
{
	...
	while( pNode ) {
		if(pNode->getNodeType() == DOMNode::ELEMENT_NODE) {
			...
			if( pXMLConfig->isAttr( pNode, "config" ) ) {
				m["%CFG%" ] = pXMLConfig->attr( pNode, "config" );
				m_mEnv[ pXMLConfig->attr( pNode, "name" ) ] = m;
			}
		}
		pNode = pXMLConfig->next( pNode );
...
void ctrl::update_state_slot() {
	...
	m_Vars["%CFG%"] =  lstr.at( lstr.indexOf( QRegExp( ".*CONF=\".*" ) ) );
	m_Vars["%CFG%"].remove( QRegExp(".*CONF=\"") ); m_Vars["%CFG%"].remove( QRegExp("\"") ); m_Vars["%CFG%"].remove( QRegExp(".*etc/") );
...
void ctrl::reportExecError( int err ) {
	...
	break;		
	case 5: {
		ui.m_pStatus_label->setText( QString("Command: %1 finished with errors:\nexit code != 0").arg(m_pCmdExecutor->last()) );
	}
	}
}
...
void ctrl::on_m_pApp_combo_currentIndexChanged(int)
{
	...
	if( m_mEnv[ui.m_pApp_combo->itemText( ui.m_pApp_combo->currentIndex() )].contains("%CFG%") )
		m_Vars["%CFG%"] = m_mEnv[ui.m_pApp_combo->itemText( ui.m_pApp_combo->currentIndex() )]["%CFG%"];
