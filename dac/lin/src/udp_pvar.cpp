...
pVariable::pVariable(CParameter *pPar) {

	setPtsID(pPar->getSHWSId());
	setDeviceID(pPar->getDevId());
	setN(pPar->getParId());
	setStatus(pPar->status());
	setDateTime(pPar->getTime());

	pValue v;
	if(pPar->getValueType() == eVT_Int)
		v.set(QString(pPar->getStringValue()).toInt());
	if(pPar->getValueType() == eVT_Double)
		v.set(QString(pPar->getStringValue()).toDouble());
	if(pPar->getValueType() == eVT_Short)
		v.set(QString(pPar->getStringValue()).toInt());
	if(pPar->getValueType() == eVT_String)
		v.set(pPar->getStringValue().toAscii().data());
