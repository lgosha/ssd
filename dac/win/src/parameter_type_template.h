...
public:
	...
	//! устанавливает состояние параметра в CParameter::ePUS_NotUpdated
			void		setNotUpdated	();
	//! запрещает обновление параметра
			void		freeze			();
	// *** флаг обновления значения
	//! возвращает необходимость обновления значения параметра
			bool		isNowUpdate		( uint )	const;
...
private:
	...
	bool						m_bFreeze;			 //! флаг запрещения обновления параметра
};
...
template<class T> void CParameterTemplate<T>::applyValue() {
	...
	if( m_bIsNew ) {
		...
		if(m_pParameterInfo->m_pProps->m_sFailValue.isNull()) {
			...
			setStatus(CParameter::ePUS_Updated);
			m_iRStatus = CParameter::ePUS_NotUpdated;
...
//! запрещает обновление параметра
template<class T> void CParameterTemplate<T>::freeze() {

	m_bFreeze = true;
}

//! возвращает необходимость обновления значения параметра
template<class T> bool CParameterTemplate<T>::isNowUpdate( uint timer )	const {

	return	(
				(
				// время задержки определения значения параметра превысило текущее время тактов (чтобы не срабатывало в при обнулении счетчика тактов)
				(int)(timer - getStart()) >= 0
				&&
				// период обновления параметра (с учетом смещения) длится без остатка на время, прощедшее от начала отсчета тактов
				!((timer - getStart()) % getUpdateTime())
				)
				||
				// безусловное обновление на данном такте
				isForceUpdate()
			)
			&&
			// параметр не заморожен
			!m_bFreeze;
}
...
template<class T> void CParameterTemplate<T>::print( QStringList &lstr ) {
	...
	lstr << QString	("%1 | %2 | %3 | %4 | %5 | %6 | %7 | %8 | %9")
			.arg( getId() )
