...
bool QtServiceBasePrivate::install(const QString &account, const QString &password, const QString &dir, const QString &conf)
{
    bool result = false;
    if (!winServiceInit())
        return result;
 ...
 /*       DWORD dwStartType = startupType == QtServiceController::AutoStartup ? SERVICE_AUTO_START : SERVICE_DEMAND_START;*/
        ...
        SC_HANDLE hService = pCreateService(hSCM, (wchar_t *)controller.serviceName().utf16(),
                                            ...
                                            dwStartType, SERVICE_ERROR_NORMAL, (wchar_t *)(filePath() + QString(" --directory %1 --config %2").arg(dir).arg(conf)).utf16(),
                                            0, 0, 0,
                                            act, pwd);
        if (hService) {
            result = true;
