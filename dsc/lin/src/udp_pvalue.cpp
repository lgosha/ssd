...
pValue::pValue(const pValue & _pv)
{
	...
	case STRING:
	    ...
	    for(int i = 0; i <= strlen(_pv.data); i++)
		data[i] = _pv.data[i];
	    break;
	case DOUBLE:
	    data = (char*)malloc(8);
...
QString pValue::toStr()
{
	...
	case pValue::INTEGER:
	    get(&vi);
		return QString("%1").arg(QString::number(vi));
	case pValue::SHORTINT:
	    get(&vi);
		return QString("%1").arg(QString::number(vi));
	case pValue::STRING:
	    get(vs);
		return QString("%1").arg(vs);
	case pValue::DOUBLE:
	    get(&vd);
		return QString("%1").arg(QString::number(vd));
    }
